...
object Updater {
    ...
    private val API_URL = "https://api.modrinth.com/v2/project/guildzip/version" +
    ...
    fun compareVersions(v1: String, v2: String): Int = try {
        ...
        GuildZipMod.LOGGER.warn("Erreur comparaison versions: ${e.message}")
        0
    }

    private fun check(): CompletableFuture<Void> = CompletableFuture.runAsync {
        runCatching { fetchLatest()?.let { latest = it } }
            .onFailure { GuildZipMod.LOGGER.error("Error checking version", it) }
    }
    ...
    private fun fetchLatest(): String? {
        ...
        conn.setRequestProperty("User-Agent", "GuildZip-Mod")
